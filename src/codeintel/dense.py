...
from codeintel.vector_index import FaissVectorIndex, VectorIndexError
    ...
    *,
    ...
    if documents:
        ...
        if vectors.ndim != 2 or vectors.shape[0] != len(documents):
            raise DenseIndexError(
                f"Embedding provider returned unexpected shape {vectors.shape} "
    ...
    with tempfile.TemporaryDirectory(dir=artifact_dir) as tmp_name:
        ...
        try:
            ...
            # Validate temporary artifacts before replacing finals.
            ...
            # Leave any previously valid final artifacts untouched.
            raise
